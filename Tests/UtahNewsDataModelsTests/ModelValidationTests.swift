...
struct ModelValidationTests {
    ...
    func testPersonMinimalData() throws {
        let person = Person(name: "John Doe", details: "Minimal person for testing")
    ...
    func testOrganizationModel() throws {
        ...
        #expect(organization.orgDescription != nil)
    ...
    func testNewsEventModel() throws {
        ...
        if let startDate = newsEvent.startDate, let endDate = newsEvent.endDate {
            #expect(startDate <= endDate, "Start date should be before or equal to end date")
        }
